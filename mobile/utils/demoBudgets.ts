...
export type DemoBudgetStatusItem = {
  id: string;
  category: string;
  monthlyLimit: number;
  spent: number;
...
const DEMO_BUDGET_SEED: DemoBudgetRow[] = [
...
let localDemoStore: DemoBudgetRow[] | null = null;

function cloneSeed(): DemoBudgetRow[] {
  return DEMO_BUDGET_SEED.map((row) => ({ ...row }));
}

function getLocalStore(): DemoBudgetRow[] {
  if (!localDemoStore) {
...
function resolveUsageStatus(usagePercent: number) {
  if (usagePercent >= 100) {
    return { status: 'exceeded' as const, message: 'Limit aşıldı' };
  }
  if (usagePercent >= 80) {
    return { status: 'warning' as const, message: 'Dikkat' };
  }
  return { status: 'safe' as const, message: 'Güvenli' };
}

export function buildDemoStatusRows(store = getLocalStore()): DemoBudgetStatusItem[] {
  return store
    .map((row) => {
...
export function upsertLocalDemoBudget(category: string, monthlyLimit: number) {
...
export function removeLocalDemoBudget(category: string) {
  const store = getLocalStore();
...
export function isBudgetDbError(err: unknown) {
  const message = String(
    (err as { message?: string; data?: { error?: string } })?.message ||
